...
`/simulate` · `/sensitivity` · `/receivables` · `/loan` · `/advise`
...
flowchart LR
    ...
    UI --> LOAN & MC & SENS & RECV & RW & CFO
    UI --> IO & STORE & SCN
    API --> LOAN & MC & SENS & RECV & RW & CFO
    UI --> PDF
    IO --> RW
    SENS --> MC
...
| `test_api.py` (13) | HTTP uçlarının sözleşmesi, girdi doğrulama ve iterasyon tavanı, **API'nin motorla aynı sayıyı vermesi**, arayüz yığını import edilemezken bile ayağa kalkması |
