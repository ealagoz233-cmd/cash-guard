...
`/simulate` · `/sensitivity` · `/receivables` · `/weekly` · `/zscore` · `/loan` · `/advise`
...
flowchart LR
    ...
    UI --> LOAN & MC & SENS & RECV & ZS & WK & RW & CFO
    UI --> IO & STORE & SCN
    API --> LOAN & MC & SENS & RECV & ZS & WK & RW & CFO
    UI --> PDF
    IO --> RW
    SENS --> MC
...
**203 test**, on dört dosyada:

| Dosya | Neyi korur |
|-------|-----------|
| `test_data_integrity.py` (31) | Mock verinin ortalamalarının skalerlerle birebir tutması, kasa yürüyüşünün sapmasız olması, eksik sütunlu CSV'nin uygulamayı çökertmemesi, Türkçe/Excel biçimlerinin okunması, Türkçe etiketler |
...
| `test_api.py` (17) | HTTP uçlarının sözleşmesi, girdi doğrulama ve iterasyon tavanı, **API'nin motorla aynı sayıyı vermesi**, arayüz yığını import edilemezken bile ayağa kalkması |
