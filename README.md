...
`/simulate` · `/sensitivity` · `/loan` · `/advise`
...
flowchart LR
    ...
    UI --> LOAN & MC & SENS & RW & CFO
    UI --> IO & STORE & SCN
    API --> LOAN & MC & SENS & RW & CFO
    UI --> PDF
    IO --> RW
    SENS --> MC
...
| `test_api.py` (11) | HTTP uçlarının sözleşmesi, girdi doğrulama ve iterasyon tavanı, **API'nin motorla aynı sayıyı vermesi**, arayüz yığını import edilemezken bile ayağa kalkması |
