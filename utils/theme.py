...
# Saf biçimlendirme utils/format.py'ye taşındı: theme.py streamlit'e bağlı ve
# HTTP API'nin sırf para biçimlendirmek için arayüz çatısı kurması saçmaydı.
# Buradan yeniden dışa aktarılıyorlar, böylece `from utils.theme import money`
# yazan mevcut kod (app.py) aynen çalışır.
from utils.format import (  # noqa: F401  (bilinçli yeniden dışa aktarım)
    EXPENSE_LABELS,
    expense_label,
    money,
    tr_num,
